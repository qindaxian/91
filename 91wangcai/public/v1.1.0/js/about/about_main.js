...
/*description：信息披露
author:chexiaoting;
update：2017-6-19 10:55:11;*/
var About=function(){
    $(".zhshuList li:nth-child(3n)").css("margin","0");
};
About.prototype={
    init:function(){
        this.initBind();
    ...
    initBind:function(){
        ...
        //点击展开收缩效果
        $(".innerMore").click(function(){
            var innerVal=$(".innerMore").text();
            $(".inner").toggleClass('moreAll');
            if(innerVal=="展开全文"){
                $(".innerMore").text("收起全文");
            }else{
                $(".innerMore").text("展开全文");
        ...
        //点击每一组证书图片显示放大图片
        $(".qualification .zhshuList li div").on("click",function(){
            $(".shade,.dialogWrap").show();
            var largeImg='<img src='+$(this).children().attr("src")+'>';//获取被选中的图片地址
            $(".dialogBox .bd").append($(largeImg));
            _that.scrollTopMain();
        });
        //点击备案信息显示放大图片
        $(".recordList li div").click(function(){
            var ctxURL=$("#ctx").val();
            console.log(ctxURL);
            $(".shade,.dialogWrap").show();
            var _index=$(this.parentNode).index()+1;
            //获取被选中的图片地址
            var largeImg='<img src='+ctxURL+'/img/about/recordBigPic'+_index+'.jpg>';
            $(".dialogBox .bd").append($(largeImg));
            _that.scrollTopMain();
        });
        //关闭放大图片层
        $(".close").click(function(){
            $(".shade,.dialogWrap").hide();
            $(".dialogBox .bd img").remove();
        });
        //点击查看证书详情
        $(".infoMore a").click(function(){
            $(".zhshuListD").toggleClass('moreAll');
            if($(this).text()=="点击查看详情 >>"){
                $(this).text("点击收起");
            }else{
                $(this).text("点击查看详情 >>");
    ...
    scrollTopMain:function(){
        var scrollTop = $(document).scrollTop();
        var scrollHeight=$(document).height();
        var windowHeight=$(window).height();
        $(".dialogWrap").css("top",scrollTop+100);
        var change=scrollHeight-windowHeight;
        if(scrollTop <= change){
            if(windowHeight < 650){
                $(".dialogWrap").css("top",scrollTop-200);
                //滚动条顶部位置小于100，放大图片弹窗位置往下
                if(scrollTop <= 200){
                    $(".dialogWrap").css("top",scrollTop+150);
                }
            }
        }
    }
