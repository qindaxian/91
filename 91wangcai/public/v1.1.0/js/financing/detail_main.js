 ...
 * @desc 项目详情页
 * @author zhangjingbo
 * @update 2015年8月31日 14:55:11
 */
var FinancDetail = function(){
    //最高出借金额
    var maxTender = parseFloat($("#maxTender").val());
    //可出借金额
    var accountWait = parseFloat($("#accountWait").val());
    var canTenderMax = maxTender > accountWait ? accountWait : maxTender;
    var canTenderMax = canTenderMax > 100 ? canTenderMax : 100;
    this.pageSize=10;
    this.pageNum=0;
}

FinancDetail.prototype = {
    init:function(){
        //初始化事件绑定
        this.initBind();
        //分页点击事件绑定
    ...
    initBind:function(){
        ...
        // 点击项目详情请求详情接口
        $("#productDe").click(function(){
            if(!$(this).hasClass("active")){
                // 获取项目详情
                _that.getProductDetails("record",_that.pageNum,_that.pageSize);
            }
        })

        $(".moneyNologin").click(function(){
            window.location.href="/user/to_login";
        })
        //表格高度自适应
        ...
        //项目概况、借款人信息、相关文件、出借信息 tab切换实现
        $(".tabHd li").click(function () {
            $(this).addClass("active").siblings().removeClass("active");//移除全部选中状态
            var index = $(".tabHd li").index(this); // 获取当前点击的li元素 在 全部li元素中的索引。
            $(".tabMain > div").eq(index).show().siblings().hide();//选中点击项

            var _href=window.location.href;
            if(_href.indexOf("prioritynew")<0){//不在优先出借详情中
                var iframe = document.getElementById("frameContent");//获取iframe
                if(index==1){
                    setAutoHeight(iframe);
                }
            }
        });
        //散标出借金额输入框购买
        $("input[name=money]").on('keyup change',function () {
            // this.value = this.value.replace(/[^\d]/gi, "");//只能输入整数，可投资元
            this.value=(this.value.match(/\d+(\.\d{0,2})?/)||[''])[0];//可输入整数或小数点，可以投资角、分
            //出借金额小于100时
            if(this.value < 100){
                // 到期本息计算
                var interest = _that.caculateInterest(this.value.length > 0 ? this.value : 0);
                $("#interest").text(interest);
                //出借金额 小于100时  抢购按钮不可点击
                $(":submit[name='submitBtn']").addClass("obtn_end").attr("disabled","disabled");
                return false;
            }
            //出借金额 大于可出借金额时  可出借金额最小为100  输入金额最后一位去掉
            if (this.value > _that.canTenderMax){
                this.value = _that.getApproTenderAccount(this.value);
                $("input[name=money]").val(this.value);
            }
            var Money=$(".tallMoney").val();//出借上限
            var tallMoney=parseFloat(Money);
            var thisValue=parseFloat(this.value);

            // var tool = new FinancTools();
            // var tallMoney=tool.dcmMul(parseFloat(parseFloat(Money).toFixed(2)),10000);
            // 校验当前输入金额大于出借上限额时不可再输入
            if(tallMoney<thisValue){
                this.value=this.value.substring(0,String(tallMoney).length)
            }
            var interest = _that.caculateInterest(this.value);
            $("#interest").text(interest);
            $(":submit[name='submitBtn']").removeClass("obtn_end").removeAttr("disabled");
        });
        //优先出借出借金额输入框购买
        $("input[name=moneyFirst]").on('keyup change',function () {
            var moneyFirstJoin = $("#moneyID").val();//出借金额
            $(".moneyFirstJoin").val(moneyFirstJoin);//出借金额的值
            var minaccount=$("#moneyID").attr("minAccount");//起投金额
            var minAccount=Number(minaccount);
            // this.value = this.value.replace(/[^\d]/gi, "");//只能输入整数，可投资元
            this.value=(this.value.match(/\d+(\.\d{0,2})?/)||[''])[0];//可输入整数或小数点，可以投资角、分
            //出借金额小于最低起投金额时
            if(this.value < minAccount){
                // 计算到期本息金额
                var interest = _that.caculateInterest(this.value.length > 0 ? this.value : 0);
                $("#interest").text(interest);
                //出借金额 小于起投金额时  抢购按钮不可点击
                $(":submit[name='submitBtn']").addClass("obtn_end").attr("disabled","disabled");
                return false;
            }
            //出借金额 大于可出借金额时  可出借金额最小为100  输入金额最后一位去掉
            if (this.value > _that.canTenderMax){
                this.value = _that.getApproTenderAccount(this.value);
                $("input[name=moneyFirst]").val(this.value);
            }
            var tallMoney=parseFloat($(".tallMoney").text());//出借上限
            var thisValue=parseFloat(this.value);
        ...
        // 债权转让相关逻辑 小于起投金额直接显示不可点击
        var moneyTransfer = $("#moneyTransfer").val();//支付金额
        var minaccount2=$(".transferMinCount").val();//起投金额
        var minAccount2=Number(minaccount2);
        var selling_account=$(".selling_account").val();//债权总价值
        //支付金额小于起投金额时
        if(moneyTransfer < minAccount2){
            var selling_account=$(".selling_account").val();//债权总价值
            if(selling_account<=minAccount2){
                // 兼容ie
                if (!Array.isArray) {
                    $("input[name=moneyTransfer]").parent().children(".form_text").remove();
                }
                // 债权本息计算
                var interest = _that.transferInterest(selling_account);
                $("#interest").text(interest);
                $("#moneyTransfer").val(selling_account);
                $("#moneyTransfer").attr("disabled","disabled")
                //可出借金额 小于起投金额时  支付按钮可点击购买
                $(":submit[name='submitBtn']").removeClass("obtn_end").removeAttr("disabled");
            }
        }
        //债权支付金额
        $("input[name=moneyTransfer]").on('keyup change',function () {
            var moneyTransfer = $("#moneyTransfer").val();//支付金额
            var minaccount=$(".transferMinCount").val();//起投金额
            var minAccount=Number(minaccount);
            var selling_account=$(".selling_account").val();//债权总价值
            var surplus_amount=$(".surplus_amount").val();//可出借金额
            // this.value = this.value.replace(/[^\d]/gi, "");//只能输入整数，可投资元
            this.value=(this.value.match(/\d+(\.\d{0,2})?/)||[''])[0];//可输入整数或小数点，可以投资角、分
            //支付金额小于100时
            if(this.value < minAccount){
                var selling_account=$(".selling_account").val();//债权总价值
                if(selling_account==surplus_amount && selling_account<=minAccount){
                    var interest = _that.transferInterest(this.value.length > 0 ? this.value : 0);
                    $("#interest").text(interest);
                    $(":submit[name='submitBtn']").removeClass("obtn_end").removeAttr("disabled");
                    return false;
                }else{
                    var interest = _that.transferInterest(this.value.length > 0 ? this.value : 0);
                    $("#interest").text(interest);
                    //支付金额 小于起投金额时  支付按钮不可点击
                    $(":submit[name='submitBtn']").addClass("obtn_end").attr("disabled","disabled");
                    return false;
                }
            }

            var interest = _that.transferInterest(this.value);
            $("#interest").text(interest);
            $(":submit[name='submitBtn']").removeClass("obtn_end").removeAttr("disabled");
        });
        $("#innerLoginBtn").click(function(){
            // _that.innerLogin();未登录状态下提示去登录弹框
            window.location.href="/user/to_login";//直接提示去登录页面
        });
        //提交表单
        //马上抢购 购买
        $(".checkStatus_payBtn").click(function(e){
            e.preventDefault();
            var _link="";
            var money = parseFloat($("#moneyID").val());//出借金额
            var tallMoney=parseFloat($(".tallMoney").val());//出借上限
            // var tool = new FinancTools();
            // var tallMoney=tool.dcmMul(parseFloat(parseFloat(Money).toFixed(2)),10000);
            if (!/\d+(\.\d{0,2})?/.test(money) || money < 100) {
                WC.Core.Dialog({
                    "content":"请输入正确的金额",
                    "btnValue":"确定",
                    "icon":"error",
            ...
            //防止2次提交
            if(!$(this).hasClass("disable")){
                Deposit.Core.checkUserIsDeposit(_link,"buy","checkStatus_payBtn",money);//购买相关按钮点击判断是否开通存管通账户
            }
        });
        // 债权转让购买进入支付页面
        $(".transferPayBtn").click(function(e){
            e.preventDefault();
            var _link="";
            var money = $("#moneyTransfer").val();//支付金额
            $(".moneyTransferVal").val(money);
            var tallMoney=Number(money);
            var minaccount=$(".transferMinCount").val();//起投金额
            var minAccount=Number(minaccount);
            var surplus_amount=$(".surplus_amount").val();//可出借金额
            var surplusAmount=parseFloat(surplus_amount);//可出借金额
            // 判断当前输入支付金额小于100的逻辑
            if (!/\d+(\.\d{0,2})?/.test(money) || money < minAccount) {
                var selling_account=$(".selling_account").val();//债权总价值
                if(selling_account<=minAccount){//债权总价值小于最低起投金额时
                    if(!$(this).hasClass("disable")){
                        Deposit.Core.checkUserIsDeposit(_link,"buyTransfer","transferPayBtn",money);//购买相关按钮点击判断是否开通存管通账户
                        return false;
                    }
                }else{
                    WC.Core.Dialog({
            ...
            // 输入支付金额大于可出借金额时提示
            if( surplusAmount < tallMoney){
                WC.Core.Dialog({
                    "content":"您的出借金额大于可出借金额！",
                    "btnValue":"确定",
                    "icon":"error",
                    "btnFn":function(){
                        WC.Core.DialogClose();
        ...
        // 优选加入计划判断开户激活授权
        $(".checkStatus_payBtn_priority").click(function(e){
            if(!$(this).hasClass("obtn_end")){
                e.preventDefault();
                var _link="";
                var money = parseFloat($("#moneyID").val());//出借金额
                var tallMoney=parseFloat($(".tallMoney").text());//出借上限
                ...
                //防止2次提交
                if(!$(this).hasClass("disable")){
                    Deposit.Core.checkUserIsDeposit(_link,"buyPriority","checkStatus_payBtn_priority",money);//购买相关按钮点击判断是否开通存管通账户
                }
            }
        });
    },
    // 获取项目详情
    getProductDetails:function(type,pageNum,pageSize){
        var _that=this;
        var priorityId=$(".priorityId").val();
        var obj={};
        obj.url="/mobile/v3/priority/tenderProduct";
        obj.params={
            page_num:pageNum,//当前页码
            page_size:pageSize,//每页个数
            priorityNid:priorityId//产品id
        };
        obj.type="get";
        obj.success=function(data){
            var _status=data.status.toString();
            var _tenderProduct=data.data.tenderProductList;
            if(_status=="200"){
                var _str=[];
                for(var i=0;i<_tenderProduct.length;i++){
                    _str.push("<tr><td><a href='/view/borrow/"+_tenderProduct[i].product_nid+"'>"+_tenderProduct[i].name+"</a></td><td>"+_tenderProduct[i].total_account+"</td><td>"+_tenderProduct[i].repay_right_time+"</td><td>"+_tenderProduct[i].redeem_next_period+"/"+_tenderProduct[i].redeem_total_period+"</td>");
                    // if(_tenderProduct[i].status==7){
                    // 	_str.push("<td>已还款</td>");
                    // }else{
                    // 	_str.push("<td>待还款</td>");
                    // }
                    // _str.push("<td><a href="+_tenderProduct[i].borrow_pawn_app_url+">协议书</a></td>")
                    _str.push("</tr>")
                }
                $("#productDetails").html(_str.join(""));
                var _page=data.page;
                var _total=_page.total,
                    _pageNum=(_page.pageNum+1),
                    _pageSize=_page.pageSize;
                //调用分页
                _that.showPage(_total,_pageNum,_pageSize,type);
            }
        };
        obj.error=function(){
    ...
    // 出借信息
    getTzMessage:function(type,pageNum,pageSize){
        var _that=this;
        var priorityId=$(".priorityId").val();
        var obj={};
        obj.url="/mobile/v3/priority/tenderRecord";
        obj.params={
            page_num:pageNum,//当前页码
            page_size:pageSize,//每页个数
            priorityNid:priorityId//产品id
        ...
        obj.success=function(data){
            var _status=data.status.toString();
            var _recordList=data.data.recordList;
            if(_status=="200"){
                var _str=[];
                for(var i=0;i<_recordList.length;i++){
                    _str.push("<tr><td>"+_recordList[i].user_name+"</td><td>"+_recordList[i].tender_account+"</td><td>"+_recordList[i].tender_time+"</td></tr>");
                }
                $("#touziMessage").html(_str.join(""));
                var _page=data.page;
                var _total=_page.total,
                    _pageNum=(_page.pageNum+1),
                    _pageSize=_page.pageSize;
    ...
    bindPager:function(){
        ...
        $("#pager").on("click","a",function(e){
            e.preventDefault();
            var _pageNum=$(this).attr("href").split("#")[1];
            _that.getProductDetails($("#pager").data("type"),_pageNum,_that.pageSize);
        });
    },
    /**
     * description: 显示分页信息
     * author：vicshang(2016-03-17 14:03:03)
     * @params total {num} 总条数
     * @params _pageNum {num} 当前页码
     * @params _pageSize {num} 每页的条数
     */
    showPage:function(total,_pageNum,_pageSize,type){
        var _num=Math.ceil(total/_pageSize),//总页数
            _start=1,//开始页码
            _end=_num;//结束页码
        var _pageStr="<table><tbody><tr><td>总共<em>"+total+"</em>条</td>";
        //判断是否有上一页和首页
        if(_pageNum>1){
            _pageStr+="<td><a href=\"#0\">首页</a></td><td><a href=\"#"+(_pageNum-2)+"\">上一页</a></td>";
        }else{
            _pageStr+="<td><span>首页</span></td><td><span>上一页</span></td>";
        }
        _start=(_pageNum>5?_pageNum-5:1);
        //判断显示的页数
        if(_pageNum>5){
            _end=(_pageNum+4)>_num?_num:(_pageNum+4);
        }else{
            _end=_num>10?10:_num;
        }
        for(var i=_start;i<=_end;i++){
            if(i==_pageNum){
                _pageStr+="<td><span>"+i+"</span></td>";

            }else{
                _pageStr+="<td><a href=\"#"+(i-1)+"\">"+i+"</a></td>";

            }
        }
        //判断是否有尾页和下一页
        if(_pageNum<_num){
            _pageStr+="<td><a href=\"#"+_pageNum+"\">下一页</a></td><td><a href=\"#"+(_num-1)+"\">尾页</a></td>";
        }else{
            _pageStr+="<td><span>下一页</span></td><td><span>尾页</span></td>";
        }
        _pageStr+="<td>共分<em>"+_num+"</em>页显示</td></tr></tbody></table>";
        $("#pager").html(_pageStr);
        $("#pager").data("type",type);
    },
    //提现校验响应
        ...
        if(data&&data.data){
            ...
            if(data.data.bindbankFlag&&data.data.bindbankFlag.toString()=="0"){
                WC.Core.Dialog({
                    ...
                    "icon":"error",
                    "btnFn":function(){
                        window.location.href="/user/recharge/to";
                    }
                });
                return;
            }
    ...
    checkPayPwd:function(){
        ...
        var obj = {
            ...
            success:function(data){
                ...
                var _pay_pwd_status=_data.pay_pwd_status;
                var _pay_pwd_reset_flag=_data.pay_pwd_reset_flag;
                var tip_msg=data.message.replace("\n","");
                if(_status==200){
                    if(_pay_pwd_status==0){//没有设置支付密码
                        art.dialog.open('/user/setup/pay_pwd_reset?refer=setup',{title: _that.checkStatus('设置支付密码'),padding:0,lock:true,height:'340px',width:'430px'});
                        return false;
                    }else{//已设置支付密码
                        if(_pay_pwd_reset_flag==0){//不需要重设密码
    ...
    //去掉字符串最后一位
    getApproTenderAccount:function(str){
        var strFloatValue = parseFloat(str);
        var newStr = str;
        while(strFloatValue > this.canTenderMax){
            var newStr = newStr.substring(0,newStr.length - 1);
            strFloatValue = parseFloat(newStr);
        }
        return newStr;
    },
    //到期本息计算
    caculateInterest:function(capital){
        capital = parseFloat(capital);
        var apr = $("#apr").text();//年利率
        var year_day_lang=$("#year_day_lang").val();//天数
        // var period = $("#period").text() / 30;//项目期限
        // var lixi=(apr * period * capital) / 12 / 100;//年利率*项目期限*出借金额/12/100
        // var interest=tool.dcmAdd(parseFloat(lixi.toFixed(2)),capital);
        var period2 = $("#period").text();//项目期限
        var lixi2=(apr/(year_day_lang*100))*period2*capital;
        var tool = new FinancTools();
        var interest=tool.dcmAdd(parseFloat(lixi2.toFixed(2)),capital);
        return parseFloat(interest).toFixed(2);
    },
    // 债权本息计算
    // 购买债权 通过支付金额推算出债权本金以及利息的公式
    // 比例N = 购买人支付金额 / 债权总价值
    // 债权本金 = 出让人债权本金 * 比例N
    // 债权本金对应整期利息 = （债权本金 * 项目投资天数 * 项目利率 ／ 100）／365
    transferInterest:function(capital){
        capital = parseFloat(capital);//支付金额
        var selling_account=$(".selling_account").val();//可出借金额 债权总价值
        var selling_capital=$(".selling_capital").val();//出让人债权本金
        var period_lang=$(".period_lang").val();//项目投资天数
        var product_per_rate=$(".product_per_rate").val();//项目利率

        var principal=capital/selling_account*selling_capital;//债权本金
        var lixi=(principal*period_lang*product_per_rate)/100/365;
        var tool = new FinancTools();
        var interest=(principal+lixi).toFixed(2);//四舍五入取2位小数
        // var interest=tool.dcmAdd(parseFloat(lixi.toFixed(2)),principal.toFixed(2));

        $(".interestTransfer").val(interest);//到期本息
        return interest;
    },
    //马上登陆
    innerLogin:function(){
        art.dialog.open('/user/inner_login',{title: '登录旺财',padding:0,lock:true,height:'300px',width:'430px'});
    },
    //校验登录状态
    checkStatus:function(title){
        var _title="登录",_cookie;
...
/**
 * 工具类
 * @update 2015-8-31 14:55:45
 */
var FinancTools = function(){
    //浮点数相加
    this.dcmAdd=function(arg1,arg2){
        var r1,r2,m;
        try{r1=arg1.toString().split(".")[1].length;}catch(e){r1=0;}
        try{r2=arg2.toString().split(".")[1].length;}catch(e){r2=0;}
        m=Math.pow(10,Math.max(r1,r2));
        return (this.dcmMul(arg1,m)+this.dcmMul(arg2,m))/m;
    };
    //浮点数相乘
    this.dcmMul=function(arg1,arg2){
...
$(document).ready(function(){
    var financDetail = new FinancDetail();
    financDetail.init();
});
// iframe自适应高度兼容
setAutoHeight=function(iframeObj){
    if  (document.getElementById){
        if(iframeObj){
            // contentDocument 属性能够以 HTML 对象来返回 iframe 中的文档  iframe自适应高度
...
window.onload=function(){
    ...
    if(_href.indexOf("prioritynew")<0){//不在优先出借详情中
        // 获取iframe高度
        var iframeId = document.getElementById("iframeId");//获取iframe
        setAutoHeight(iframeId);
    }
};
// 元转化为万元保留两位小数
...
TenThousand.prototype = {
    init: function () {
        $(".tenThousand").each(function () {
            var num = Number($(this).text());
            var num1 = Math.floor((num % 10000) / 1000);
            var num2 = Math.floor((num % 1000) / 100);
            var num3 = Math.floor(num / 10000);
            var text = num3 + "." + num1 + num2;
            $(this).text(text)
        })

    }
}
var _tenThousand = new TenThousand()
_tenThousand.init();
