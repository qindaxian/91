 ...
 * @desc 帮助中心
 * @author zhangjingbo
 * @update 2015-9-6 15:17:10
 */
var HelpObj = function(){};
HelpObj.prototype = {
    init:function(){
        //this.showDefault();//默认内容展示
        this.initBind();//事件绑定
    },
    //初始化 事件绑定
    initBind:function(){
        ...
        $('#transferBtn').on('click',function(){
            $(".zhezhao").removeClass("hidd");
            $(".bgBox").removeClass("hidd");
            $(".conBox").removeClass("hidd");
        });
        //弹出框关闭按钮
        $("#closeBtn").on("click",function(){
            $(".zhezhao").addClass("hidd");
            $(".bgBox").addClass("hidd");
            $(".conBox").addClass("hidd");
        });
        //导航栏事件绑定
        $('.tab').on('click', '.item',function () {
            var _index = $(this).index();
            $(this).addClass('active').siblings().removeClass('active');
            $('.item_list:eq('+_index+')').removeClass('hidden').siblings().addClass('hidden');
            console.log($('.item_list:eq('+_index+')').html());

        });
        // $('.tab').on('click','.item1',function(){
        // 	$('.explain').css('border-bottom','none');
        // 	$('.appear').css('display','block');
        // 	var _index=$(this).index();
        // 	$(this).siblings().removeClass('hover').children('.item-list').hide();
        // 	$(this).siblings().addClass('bo');
        // 	$(this).css('border-top','none');

        // 	$(this).addClass('hover');
        // 	$('.item-list').css('display','none');
        // 	$('.item-list:eq('+_index+')').css('display','block');
        // });
        //条目 打开/关闭事件绑定
        // $('.text h2').on('click',function(){
        // 	$(this).parents().siblings().children('h2').next().hide();
        // 	$(this).parents().siblings().children('h2').children('span').removeClass("reduce").addClass('add');

        // 	if(!$(this).children('span').is(".reduce")){
        //         $(this).children('span').addClass("reduce").removeClass("add");
        //         $(this).next().slideToggle();
        //     }else{
        //         $(this).children('span').removeClass("reduce").addClass("add");
        //         $(this).next().slideToggle();
        //     }
        // });
    },
    ...
    // 	var _that=this;
    // 	var _source=_that.getQueryString("source");
    // 	if(_source=="jrPc"){
    // 		$(".item:first-child").removeClass("bo");
    // 		$(".item:first-child").removeClass("white");
    // 		$(".item:nth-child(8)").addClass("hover");
    // 		$(".item-list:nth-child(8)").css('display','block').siblings().css('display','none');
    // 	}else{
    // 		$('.show_text').show();//内容展示
    // 		$('.white').addClass('hover');//左侧导航栏添加选中样式
    // 		$('.tab .hover h3').css('border-top','none');//去除选中 上边界线

    // 		$('.content').hide();//隐藏条目项目信息
    // 		$('.text h2').children('span').addClass('add');//所有条目 添加打开按钮
    // 		//让第一个的内容显示
    // 		$($('.subitem .text:first-child> .content')).show().siblings().children('span').removeClass('add').addClass('reduce');
    // 	}
