 ...
 * @desc 我的账户 账户设置
 * @author zhangjingbo
 * @update 2015-9-6 14:52:59
 */
var UserSet = function(){
	this.init();
	this.pay_pwd_status;
	this.tip_msg;
	this.pay_pwd_reset_flag;
	this.isDeposit;
	this.isUnBindCard;
	this.isActivate;
	this.userphone;
	this.isautoTenderDialog;
...
UserSet.prototype = {
	init:function(){
		//初始化事件绑定
		this.initBind();
		this.checkIsDeposit("bank");//判断用户是否开户、绑卡
	},
	//初始化事件绑定
	initBind:function(){
		var _that = this;
		...
		$(".dialogClose").click(function(){
			$("#setPayPwd_vic").addClass("hidden");
			$("#setBankPhone").addClass("hidden");
		});
	    //点击修改交易密码
	    $("#login_edit_pwd").click(function(){
	    	if(_that.isDeposit==0){
	    		WC.Core.Dialog({
					"content":"您尚未开通厦门银行资金存管账户，请先开通。",
					"btnValue":"立即开通",
					"btn2Value":"暂不开通",
	    	...
	    	}else if(_that.isDeposit==1){
	    		if(_that.isActivate == 0){//未激活
					WC.Core.Dialog({
						"content":"您的账户已升级为厦门银行资金存管账户，请您激活账户。",
						// "close":true,
						"btnValue":"确定",
					...
					Deposit.Core.resetPassword();//修改或忘记交易密码
				}
	    	}else if(_that.cunguanDialog==314){//存管通停服
	    		WC.Core.Dialog({
					"content":_that._announcement,
					"type":"cunguan",
					"close":true,
					"cgTitle":_that._cunguantitle
				});
	    	}
		});
	    ...
	    $("#login_edit").click(function(){
	        art.dialog.open('/user/setup/login_edit_pwd?refer=setup',{title: _that.checkStatus('修改登录密码'),padding:0,lock:true,height:'284px',width:'430px'});
	    });
	    // $("#add_email").click(function(){
	    //     art.dialog.open('/user/setup/edit_email?refer=setup',{title: _that.checkStatus('邮箱认证'),padding:0,lock:true,height:'268px',width:'430px'});
	    // });
	    // $("#edit_email").click(function(){
	    //     art.dialog.open('/user/setup/edit_email?refer=setup',{title: _that.checkStatus('修改邮箱认证'),padding:0,lock:true,height:'248px',width:'430px'});
	    // });
		//点击实名按钮事件
	    ...
	    $("#edit_phone").click(function(){
	        art.dialog.open('/user/setup/edit_phone_1?refer=setup',{title: _that.checkStatus('修改手机号码'),padding:0,lock:true,height:'400px',width:'430px'});
	    });
	    //点击绑定按钮进行开户
	    $("#bank_id").click(function(){
	    	if(_that.isDeposit==0){
	    		...
	    		if(_that.isActivate == 0){//未激活
					...
					if(_that.isUnBindCard==1){//已解绑
						Deposit.Core.bindBankcard();//重新绑定银行卡
		    		}else{
		    			WC.Core.Dialog({
			                "content":"解绑请联系在线顾问400-000-0091",
			                "btnValue":"确定",
			                "icon":"ok",
			                "btnFn":function(){
			                    WC.Core.DialogClose();
		    			...
		    			// Deposit.Core.unbindBankCard();//解绑银行卡
		    		}
				}
	    	}else if(_that.cunguanDialog==314){//存管通停服
	    		WC.Core.Dialog({
	    	...
	    	if(_that.isDeposit==0){
	    		...
	    		if(_that.isActivate == 0){//未激活
					...
					if(_that.isUnBindCard==1){//未绑卡
		    			...
		    			if(_that.authenticationFlag==0){//鉴权未通过/未鉴权
		    				WC.Core.Dialog({
				                "content":"您的银行卡未通过验证，请解绑后再重新绑卡。解绑请拨打400-000-0091",
				                "btnValue":"确定",
				                "icon":"error",
				                "btnFn":function(){
				                    WC.Core.DialogClose();
		    				...
		    				$("#BankPhone").val("");//清空输入框内容
		    				$("#setBankPhone").removeClass("hidden");//填写新预留手机号弹框显示
		    			}
		    		}
				}
	    	}else if(_that.cunguanDialog==314){//存管通停服
	    ...
	    //点击更新预留手机号弹框中的确定按钮
	    $("#setBankPhone_sub_oBtn").click(function(){
	    	var _newBankPhone = $.trim($("#BankPhone").val());//获取新的预留手机号码
	    	if(_newBankPhone == ''){
                $(".errorTip").text("请输入手机号!");
                return;
            }
            if(!/^1[3456789]\d{9}$/.test(_newBankPhone)){
                $(".errorTip").text("手机号输入错误！");
                return;
            }
            _that.changeBankPhone(_newBankPhone);//更新银行预留手机号
	    });
	    //预留手机号输入框获取焦点
	...
	//更新银行预留手机号
	changeBankPhone:function(_mobile){
		var obj={};
		obj.url="/xiamen/deposit/modifyUserBankPhone";
		obj.type="post";
		obj.params={"userDevice":"PC","mobile":_mobile};//用户设备（PC：PC 端，MOBILE：移动端） mobile修改后用户的预留手机号
		obj.success=function(data){
			$("#setBankPhone").addClass("hidden");
			var _status=data.status.toString();
			var _data=data.data;
	...
	//判断是否开通存管通账户,决定页面展示字段
	checkIsDeposit:function(_type){
		var _that=this;
		var obj={};
		obj.url="/xiamen/deposit/isDeposit";
		obj.type="post";
		...
		obj.success=function(data){
			var _status=data.status.toString();
			if(_status == "0"){
				var isDeposit = data.data.isDeposit,//是否开通存管账号 0：未开通，1：已开通
					isActivate = data.data.isActivate,//{0:未激活；1:已激活}
					isUnBindCard = data.data.isUnBindCard,//是否绑定银行卡 0：未解绑，1：已解绑
					isAllowMiddleUserHandle = data.data.isAllowMiddleUserHandle,//是否居间人用户，如果是居间人则只允许居间人充值、提现 0：不允许，1：允许
					autoTenderDialog = data.data.autoTenderDialog;//是否提示失败 0：不提示失败，1：提示
					authenticationFlag=data.data.authenticationFlag;//0：鉴权未通过/未鉴权，1：4要素鉴权通过，2：3要素鉴权通过
					_that.authenticationFlag=data.data.authenticationFlag;
					_that.isUnBindCard=data.data.isUnBindCard;
					_that.isDeposit=data.data.isDeposit;
					_that.isActivate = data.data.isActivate;//{0:未激活；1:已激活}
					_that.isautoTenderDialog=data.data.autoTenderDialog;//是否弹框提示自动出借失效
					if(isDeposit == 0){//未开户
						//交易密码字段显示设置
						$("#pwdStatus").html('<i class="ico ico_alert"></i>未设置');
    					$("#login_edit_pwd").html("设置");
    					$("#paypwdBtn span,#cz_pwd").html("");
					}else{//已开户
						if(isActivate == 0){//未激活
							//交易密码字段显示设置
							$("#pwdStatus").html('<i class="ico ico_alert"></i>未设置');
							...
							//实名认证字段显示设置
	    					$("#identityStatus").html('<i class="ico ico_over"></i>已认证');
	    					$("#add_id").html('');
	    					_that.getRealname();//展示实名信息
							if(isUnBindCard == 0){//已绑卡
								//绑定银行卡
								$("#bindbankStatus").html('<i class="ico ico_over"></i>已认证');
								$("#bank_id").html("解绑");
								//预留手机号
								$("#bankTelStatus").html('<i class="ico ico_over"></i>已设置');
								$("#bankTel_id").html("更新");
								_that.getBankInfo();//展示绑卡信息、预留手机号信息
							}
						}
					}
			}else if(_status=="314"){//存管停服返回说明
				var _announcement=data.data.announcement;
				var _cunguantitle=data.data.title;
				_that.cunguanDialog=314;
				_that._announcement=_announcement;
				_that._cunguantitle=_cunguantitle;
				WC.Core.Dialog({
					"content":data.data.announcement,
					"type":"cunguan",
					"close":true,
	...
	getRealname:function(){
		...
		obj.success=function(data){
			var _status=data.status.toString();
			if(_status == "0"){
				var authDate = data.data.authDate,//实名时间
					cardId = data.data.cardId,//身份证号
					realname = data.data.realname,//真实姓名
					sex = data.data.sex;//性别
				$("#identity").html(realname+'<div>'+cardId+'</div>');
			}else if(_status == "-1"){
				window.location.href="/user/to_login";
			}else{
				WC.Core.Dialog({"content":data.message,"icon":"error"});
	...
	getBankInfo:function(){
		var _that=this;
		var obj={};
		obj.url="/xiamen/deposit/getBankInfo";
		obj.type="get";
		obj.params={};
		obj.success=function(data){
			var _status=data.status.toString();
			if(_status == "0"){
				var addtime = data.data.addtime,//绑卡时间
					icno = data.data.icno,//身份证号
					realname = data.data.realname,//真实姓名
					bank = data.data.bank,//银行名称
					userphone = data.data.userphone,//银行预留手机号
					cardno = data.data.cardno;//银行卡号
				_that.userphone = userphone;
				$("#bankMsg").html(bank+'：'+cardno);
				$("#bankTel").html("您的预留手机号："+userphone);
			}else if(_status == "300"){
				window.location.href="/user/to_login";
			}else{
				WC.Core.Dialog({"content":data.message,"icon":"error"});
