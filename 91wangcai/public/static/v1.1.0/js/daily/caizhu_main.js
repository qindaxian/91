...
/*description：财主见面会
author:liqian;
update：yangzhangobo 2017:08:11 10:55:11;
*/
var Caizhu = function () {
	this.pageSize = 12;
	this.pageNum = 0;
...
Caizhu.prototype = {
	init: function () {
		var _that = this;
		this.initBind();
		this.getBannerTop();//获取banner图片及文字介绍
		this.getData();//获取tab年份
		this.getListData("", _that.pageNum, _that.pageSize);
		this.bindPager();//分页点击事件绑定

	},
	initBind: function () {
		// 滑过显示阴影框
		$(document).on('mouseenter', '.listYear li', function () {
			$(this).stop().animate({"margin-top":"-5px","margin-bottom":"25px",}, 300).find(".shade").stop().animate({"opacity":"1"},300);
		});
		$(document).on('mouseleave', '.listYear li', function () {
			$(this).stop().animate({"margin-top":"0px","margin-bottom":"20px"}, 300).find(".shade").stop().animate({"opacity":"0"},300);
		});
	},
	// 获取tab
	getData: function () {
		...
		obj.params = {
			"type": 2,
			"page_size": _that.pageSize,
			"page_num": _that.pageNum,
			"year": ""
		};
		obj.type = "get";
		obj.success = function (data) {
			...
			var _data = data.data;
			if (_status == '200') {
				$(".loadWrap").addClass("hidden");//获取数据后隐藏loading
                $(".wrapper").removeClass("hidden");//显示页面
				// 定义字符串
				var _list1 = "";
				var _list2 = "";
				var _str = [];
				...
				// 获取年的个数 生成tab个数
				for (var i = _data.year.length - 1; i >= 0; i--) {
					var _item = _data.year[i].time;
					_list1 += _that.tabswitch(_item);
					_strli.push('<li id="' + _item + '" class=""></li>');
				}

				/*添加几个年份的tab*/
				$("#dates").html(_list1);
				/*添加对应年份的li盒子*/
				$("#dates li:first-child").addClass("active");//最新一年高亮
				//列表tab点击切换
				$('#dates>li').click(function () {
					$("#dates>li").removeClass("active");
					$(this).addClass("active");
					var str = $(this).text();
					_that.getListData(str, _that.pageNum, _that.pageSize);
				});
			}
		};
		obj.error = function () { };
	...
	// 顶部tab切换动态获取年份
	tabswitch: function (datetime) {
		var _that = this;
		var list1 = '<li>' + datetime + '</li>'
		return list1;
	},
	// 获取头部图片以及介绍文字
	getBannerTop: function () {
		...
		obj.success = function (data) {
			var _status = data.status.toString(),
				_data = data.data,
				_messageInfo = _data.messageInfo[0],
				// _title=_messageInfo.title,//图片上的字
				_img = _messageInfo.file_url[0];
			if (_status == 200) {
				// $(".bannerText").text(_title);
				$(".bannerImg").attr("src", _img);
			}
		};
		obj.error = function () { };
		WC.Core.ajax(obj);
	...
	//获取2014-1016财主见面会数据
	getListData: function (dateyaer, pageNum, pageSize) {
		var _that = this;
		var obj = {};
		obj.url = "/meeting_diary";
		obj.params = {
		...
		obj.success = function (data) {
			...
			var _data = data.data;
			if (_status == '200') {
				var _data = data.data.messageInfo;
				var _str = [];
				//循环遍历获取参数，返回的应该是一个数组
				for (var i = 0; i < _data.length; i++) {
					if ((i + 1) % 3 == 0) {
						_str.push('<li class="last"><div class="shade"></div><div class="info"><a href="' + _data[i].content_url + '" target="_blank"><img width="240" height="225" src="' + _data[i].file_url[0] + '" class="imgloadinglater"></a><h4>' + _data[i].contents + '</h4><p class="time">' + _data[i].title_time + '</p></div></li>');
					} else {
						_str.push('<li><div class="shade"></div><div class="info"><a href="' + _data[i].content_url + '" target="_blank"><img width="240" height="225" src="' + _data[i].file_url[0] + '" class="imgloadinglater"></a><h4>' + _data[i].contents + '</h4><p class="time">' + _data[i].title_time + '</p></div></li>');
					}
				}
				$("#listYear").html(_str.join(""));
				...
				// 调用分页
				_that.showPage(_total, _pageNum, _pageSize, dateyaer);
			}
		};
		obj.error = function () { };
		WC.Core.ajax(obj);
	...
	bindPager: function () {
		...
		$("#pager").on("click", "a", function (e) {
			...
			_that.getListData($("#pager").data("type"), _pageNum, _that.pageSize);
		});
	},
	/**
	* description: 显示分页信息
